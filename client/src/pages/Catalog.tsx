...
export const Catalog = observer(() => {
  ...
  .then((amount) => {
    setQuizzesAmount(amount);
      ...
      if (page * 10 > quizzesAmount) {
        getQuizzes((page - 1) * 10, quizzesAmount).then((quizzes) => {
  ...
  return (
    <PageLayout className="relative flex flex-col items-center h-auto min-h-screen">
      ...
      <div className="grid grid-cols-2 gap-[48px] mt-[84px] mb-[128px]">
        {quizElements}
      </div>

      <div className="absolute bottom-0 flex items-center justify-center">
        <CatalogNavigate currentPage={page} totalPages={(quizzesAmount + 9) / 10} />
      </div>
    </PageLayout>
  );
});
